...
[← Retour à la page précédente](javascript:history.back())
...
Retrouver le [générateur de métadonnées](download.html#meta) dans la section Ressources.

#### 3.3.1 La méthode simple (recommandée)

Insérez une ou plusieurs balises `<meta>` dans l’en‑tête (`<head>`) de votre page HTML. La structure est conçue pour **lisible, extensible et indépendante du type de contenu**.

**Logique des métadonnées**

- **Clé de contenu variable :** elle identifie le type de média (ex : `editorial`, `visual`, `code`). C’est la seule partie qui change selon le composant labellisé.
- **Suffixes figés :**
  - `content-origin` - origine du **Fond** (intention, idées, structure)
  - `form-origin` - origine de la **Forme** (formulation, exécution)
  - `ai-role` - rôle précis joué par l’IA
- **Valeurs normalisées :** les tableaux ci‑dessous détaillent les valeurs admises pour chaque label.

**Tableau des origines par label**

| Label | `content-origin` (Fond) | `form-origin` (Forme) | `ai-role` |
|-------|-------------------------|-----------------------|------------|
| **HUC** | `human` | `human` | `none` |
| **HCA** | `human` | `human-corrected` | `technical-correction` |
| **HCE** | `human` | `ai-assisted` | `assisted-formulation` |
| **ACE** | `hybrid` | `hybrid` | `co-construction` |
| **AIC** | `synthetic` | `ai-generated` | `generation` |

**Clés de contenu (Content Keys)**

| Content Key | Usage typique |
|-------------|---------------|
| `editorial` | Article, billet de blog, essai, rapport, documentation textuelle |
| `visual` | Image, illustration, infographie, logo, photographie |
| `code` | HTML, CSS, JavaScript, script, code source d’un programme |
| `audio` | Podcast, musique, bande-son, enregistrement vocal |
| `video` | Vidéo, animation, film, séquence animée |
| `data` | Tableau de données, jeu de données, graphique basé sur des données |
| `transcription` | Transcription automatique d’un audio ou d’une vidéo |
| `translation` | Version traduite d’un contenu existant |
| `synthesis` | Résumé, synthèse automatique d’un document |

**Exemples de blocs de métadonnées**

*Contenu unique (visuel en AIC)*

```html
<head>
    <meta name="tcp-up:labels" content="visual:AIC">
    <meta name="tcp-up:visual-content-origin" content="synthetic">
    <meta name="tcp-up:visual-form-origin" content="ai-generated">
    <meta name="tcp-up:visual-ai-role" content="generation">
</head>
```

*Contenu composite (éditorial HCE + code ACE)*

```html
<head>
    <meta name="tcp-up:labels" content="editorial:HCE, code:ACE">

    <meta name="tcp-up:editorial-content-origin" content="human">
    <meta name="tcp-up:editorial-form-origin" content="ai-assisted">
    <meta name="tcp-up:editorial-ai-role" content="assisted-formulation">

    <meta name="tcp-up:code-content-origin" content="hybrid">
    <meta name="tcp-up:code-form-origin" content="hybrid">
    <meta name="tcp-up:code-ai-role" content="co-construction">
</head>
```
